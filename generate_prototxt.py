...
def vgg_16(lmdb, bs_train, bs_val, rate, lmdb_flag, not_deploy=True):
    n = caffe.NetSpec()
    if not_deploy:
        if lmdb_flag:
    ...
    n.fc6_conv, n.relu6 = conv_relu(n.pool5, nout=4096, pad=0, ks=7)
    n.drop6 = L.Dropout(n.relu6, dropout_ratio=0.5, in_place=True)
    n.fc7_conv, n.relu7 = conv_relu(n.relu6, nout=4096, pad=0, ks=1)
    n.drop7 = L.Dropout(n.relu7, dropout_ratio=0.5, in_place=True)

    n.fc8_conv = L.Convolution(n.relu7, kernel_size=1, num_output=1000)
    if not_deploy:
        n.loss = L.SoftmaxWithLoss(n.fc8_conv, n.label)
        n.acc_top_1 = L.Accuracy(n.fc8_conv, n.label, top_k=1)
        n.acc_top_5 = L.Accuracy(n.fc8_conv, n.label, top_k=5)
    else:
        n.prob = L.Softmax(n.fc8_conv)
...
def solver_and_prototxt(compress_layer, compress_rate):
    ...
    pt_folder = osp.join(osp.abspath(osp.curdir) +
                         '/' + candidate[compress_layer])
    ...
    with open(candidate[compress_layer] + '/trainval.prototxt', 'w') as f:
        f.write(
            vgg_16('/opt/luojh/Dataset/ImageNet/leveldb/', bs_train=32, bs_val=10, rate=compress_rate_new, lmdb_flag=False))

    with open(candidate[compress_layer] + '/old.prototxt', 'w') as f:
        f.write(
            vgg_16('/opt/luojh/Dataset/ImageNet/leveldb/', bs_train=32, bs_val=1, rate=compress_rate_old, lmdb_flag=False, not_deploy=False))
